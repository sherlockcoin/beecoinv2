...
unsigned int nTargetSpacing = 30 * 1; // BeeCoinV2: every 30 sec
unsigned int nRetarget = 1;
unsigned int nStakeMinAge = 60 * 60 * 24 * 7;	// minimum age for coin age: 7d
unsigned int nStakeMaxAge = 60 * 60 * 24 * 14;	// stake age of full weight: 15d
unsigned int nModifierInterval = 10 * 60; // time to elapse before new modifier is computed
static const int64_t nTargetTimespan_legacy = nTargetSpacing * nRetarget; // every 1 block
static const int64_t nInterval = nTargetTimespan_legacy / nTargetSpacing;
int64_t devCoin = 2000 * COIN;
...
const string strMessageMagic = "BeeCoinV2 Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    {
        nSubsidy = 26909201049 * COIN;
    }
		else if(pindexBest->nHeight < 499000)
    {
		nSubsidy = 20000 * COIN;
	}
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    if(pindexBest->nHeight < 7 * DAILY_BLOCKCOUNT)
        ...
        nRewardCoinYear = 1 * 365 * MAX_MINT_PROOF_OF_STAKE;
    else if(pindexBest->nHeight < (45 * DAILY_BLOCKCOUNT))
        nRewardCoinYear = 0.2 * 365 * MAX_MINT_PROOF_OF_STAKE;
...
static const int64_t nTargetTimespan = 30 * 30;  // 15 minutes
//
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BeeCoinV2", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "BeeCoinV2 Back to the Moon 13/05/2014";
        CTransaction txNew;
        txNew.nTime = 1399929277;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1399929277;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3472840;
        if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("703762aa6301c8488b0fb356762755683be006cee43f3c37c25a47add319c93f"));
        block.print();
